...
import com.ulink.ulink.Ulink.DialogBuilder2
...
fun reportNoticeDialog(view : View, idx : Int){
    ...
    choiceDialog.layout.findViewById<TextView>(R.id.tv_accuse).setOnClickListener {
        val body = RequestNoticeReport(noticeIdx = idx, reason = reason, content = content)
        RetrofitService.service.reportNotice(DataRepository.token, body).enqueue(object : Callback<ResponseNotice>{
